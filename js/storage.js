...
// UI Helper to open settings
function openSettings() {
    const owner = prompt("GitHub Username:", ghStorage.config.owner);
    if(owner === null) return;
    const repo = prompt("Repository Name (e.g., CatSentry):", ghStorage.config.repo);
    const token = prompt("Personal Access Token (needs 'repo' or 'contents:write' scope):", ghStorage.config.token);

    if (owner && repo && token) {
        ghStorage.saveConfig(owner, repo, token);
        updateLog("GitHub settings saved! Attempting sync...");
        syncWithCloud();
    }
}
...
async function syncWithCloud() {
    ...
    if (remoteData) {
        // 2. Merge Strategies
        // Simple strategy: Combine arrays and dedup by timestamp+type
        // This handles if "Laptop" added an event and "Phone" added an event.
        ...
        saveData(); // Save merged to local
        updateUI();
    }
    
    // 3. Push back the merged state (so the cloud is up to date too)
